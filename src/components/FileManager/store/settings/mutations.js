...
export default {
    ...
    manualSettings(state, data) {
        ...
        let historyFolder = localStorage.getItem('lastSelectedPath');

        if (historyFolder === '') {
            historyFolder = null
        }

        state.startFolder = data.startFolder || historyFolder || null;
        state.folderBack = data.folderBack || null;
        state.startSelection = data.startSelection || null;
        state.startDisk = data.startDisk || historyDisk || null;
    },
        ...
        if (!state.baseUrl) {
            if (process.env.VUE_APP_LFM_AXIOS_BASE_URL) {
                ...
                if (window.location.port.length) {
                    baseUrl += `:${window.location.port}/api/filemanager/`;
                } else {
