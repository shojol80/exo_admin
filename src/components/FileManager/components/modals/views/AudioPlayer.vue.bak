<template>
    <b-modal modal-class="modal-primary modal-min-btn fm-modal-audio-player"
             centered
             ref="modal"
             :size="modalSize"
             v-model="showModal"
        ...
        setSource(index) {
            this.player.source = {
                ...
                sources: [{
                    ...
                    src: `${this.$store.getters['fm/settings/baseUrl']}getFile?disk=${this.selectedDisk}&path=${encodeURIComponent(this.audioFiles[index].path)}`,
