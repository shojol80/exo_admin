<template>
  <div :class="['nestable', `nestable-${group}`, rtl ? 'nestable-rtl' : '']">
    <ol class="nestable-list nestable-group">
      ...
      <!-- Render items -->
      <template v-for="(item, index) in items" :key="item[keyProp]">
        <NestableItem
          :index="index"
          :item="item"
          :options="itemOptions">
  ...
  export default {
    ...
    watch: {
      collapsedItems(value){
        this.bus.$emit('toggle-collapse',value)
      },
      value: {
        deep: true,
        immediate: true,
        handler() {
          this.items = listWithChildren(this.value, this.childrenProp)
          this.isDirty = false
        }
      }
    },
    ...
    methods: {
      createEventBus() {
        // Simple event bus for Vue 3 compatibility
        const events = {};
        return {
          $on(event, callback) {
      ...
      moveItem({dragItem, pathFrom, pathTo}) {
        ...
        this.items = update(this.items, removePath)
        this.items = update(this.items, insertPath)
