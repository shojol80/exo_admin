  ...
  <li :class="itemClasses">
    <div
      ...
      <slot
        ...
        :item="item"
        :isChild="isChild"
        :collapsed="collapsedNested"
        :hasChildren="hasChildren"
        :collapse="onCollapseNested"
      />
    </div>
    ...
    <ol
      v-if="hasChildren"
      class="nestable-list"
      :class="{collapsed:collapsedNested}">
      <template
...
export default {
  ...
  inject: ['listId', 'group', 'keyProp', 'bus'],
  ...
  created() {
    this.bus.$on('toggle-collapse', value => this.collapsedNested = value)
  },
  computed: {
    isDragging() {
  ...
  methods: {
    onCollapseNested(event) {
      this.collapsedNested = !this.collapsedNested
    },
    onMouseEnter(event) {
